...
function GroupChatMessages({ socket, userData, groupId }) {
	...
	return (
		<>
			...
			<div
				id="chatBox"
				className="h-44 p-2 overflow-y-auto flex flex-col bg-white rounded border-2 border-lightgray"
			>
				{messageDiv.length > 0 ? (
					messageDiv
				) : (
					<div className="italic text-darkgrey">Start a conversation </div>
				)}
			</div>
		</>
	);
